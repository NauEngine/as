...
Сейчас для полчения интерфейса для реализации скриптового модуля используется
два способа:
- Получение интерфейса из самого файла скрипта (во время AOT компиляции или во
время перезагрузки)
- Получение интерфейса из шаблонов (во время первой JIT компиляции)

Собственно перестать использовать интерфейс из шаблонов. На раз он все равно
есть - использовать его для проверки типов, например

### Развитие функционала

#### Горячая перезагрузка на разных платформах

Горячая перезагрузка файлов должна стать частью функционала `CoreCompile`. Так
как перезагрузка применима только к режиму JIT компиляции и `CoreCompile`
работает как раз с нормальными представлениями файлов.

С учетом того, что реализация слежения за изменениями в файловой системе
является платформозависимой это надо будет реализовать в виде стратегий, с
выбором корректной во время компиляция или выполнения

#### Поддержать загрузку нескольких файлов для одного скриптового модуля

Собственно один скриптовый модуль, предоставляющий функционал в проект, может
зависить от других, "внутренних" скриптовых модулей. При загрузке "главного"
модуля необходимо в `CoreCompile` сообщать имена загруженных модулей, чтобы
при изменение этих файлов, происходила перезагрузка.

Дальнешее развитие - перенести логику чтения файлов в `CoreCompile`, а
реализация поддержки языков будет работать исключительно с содержанием файлов.

#### Проверка типов во время JIT компиляции и перезагрузки

В моменты JIT компиляции (как первая компиляция, так и перезагрузка), у нас есть
требуемый интерфейс модуля. С другой стороны, у нас есть интерфейс реализуемый
модулем. Эти интерфейсы должны совпадать. Можно проверить.
